...
static const char * const *riscv_la_core_names;
...
set_default_riscv_dis_options (void)
{
  ...
  riscv_la_core_names = riscv_la_core_names_abi;
  no_aliases = 0;
}
...
parse_riscv_dis_option (const char *option)
{
  if (strcmp (option, "no-aliases") == 0)
  ...
  else if (strcmp (option, "numeric") == 0)
    {
      ...
      riscv_la_core_names = riscv_la_core_names_numeric;
    }
  else
    {
      /* Invalid option.  */
...
print_insn_args (const char *d, insn_t l, bfd_vma pc, disassemble_info *info)
{
  ...
  case 'X': // LACore extension
    switch (*++d) {
      case 'a':
        print (info->stream, "%s", 
          riscv_gpr_names[EXTRACT_OPERAND(LRXA, l)]);
        break;
      case 'b':
        print (info->stream, "%s", 
          riscv_gpr_names[EXTRACT_OPERAND(LRXB, l)]);
        break;
      case 'c':
        print (info->stream, "%s", 
          riscv_gpr_names[EXTRACT_OPERAND(LRXC, l)]);
        break;
      case 'f':
        print (info->stream, "%s", 
          riscv_fpr_names[EXTRACT_OPERAND(LRFA, l)]);
        break;
      case 'A':
        print (info->stream, "%s", 
          riscv_la_core_names[EXTRACT_OPERAND(LRA, l)]);
        break;
      case 'B':
        print (info->stream, "%s", 
          riscv_la_core_names[EXTRACT_OPERAND(LRB, l)]);
        break;
      case 'C':
        print (info->stream, "%s", 
          riscv_la_core_names[EXTRACT_OPERAND(LRC, l)]);
        break;
      case 'D':
        switch (*++d) {
          case ',':
            print (info->stream, "%s", 
              riscv_la_core_names[EXTRACT_OPERAND(LRD, l)]);
            print (info->stream, "%c", *d);
            break;
          case 'm':
            loc = EXTRACT_OPERAND(LRDLOC, l);
            print (info->stream, "%s%s%s", 
              (loc == LA_LOC_MEM ? "(" : (loc == LA_LOC_SCH ? "{" : "")),
              riscv_la_core_names[EXTRACT_OPERAND(LRD, l)],
              (loc == LA_LOC_MEM ? ")" : (loc == LA_LOC_SCH ? "}" : ""))
            );
            break;
          default:
            print (info->stream, _("# LACore error, undefined modifier (%c)"),
             *d);
            return;
        }
        break;
