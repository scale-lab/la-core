    ...
    switch (c = *p++) {
      case 'C': /* RVC */
        switch (c = *p++) {
          ...
          default:
            as_bad (_(
              "internal: bad RISC-V opcode (unknown operand type `C%c'): %s %s"),
              c, opc->name, opc->args);
            return FALSE;
        }
        break;
      ...
      case 'X': 
        switch (c = *p++) {
          case 'a': USE_BITS (OP_MASK_LRXA, OP_SH_LRXA); break;
          case 'b': USE_BITS (OP_MASK_LRXB, OP_SH_LRXB); break;
          case 'c': USE_BITS (OP_MASK_LRXC, OP_SH_LRXC); break;
          case 'f': USE_BITS (OP_MASK_LRFA, OP_SH_LRFA); break;
          case 'A': USE_BITS (OP_MASK_LRA,  OP_SH_LRA); break;
          case 'B': USE_BITS (OP_MASK_LRB,  OP_SH_LRB); break;
          case 'C': USE_BITS (OP_MASK_LRC,  OP_SH_LRC); break;
          case 'D': 
            USE_BITS (OP_MASK_LRD,  OP_SH_LRD); 
            switch (c = *p++) {
              case 'm': USE_BITS (OP_MASK_LRDLOC,  OP_SH_LRDLOC); break;
              case ',': break;
              default:
                as_bad (_(
                  "internal: bad LACore opcode (unknown operand `C%c'): %s %s"),
                  c, opc->name, opc->args);
                return FALSE;
            }
            break;
          default:
            as_bad (_(
              "internal: bad RISC-V opcode (unknown operand type `X%c'): %s %s"),
              c, opc->name, opc->args);
            return FALSE;
        }
        break;
...
md_begin (void)
{
  ...
  //LACore extension 
  hash_reg_names (RCLASS_LAR, riscv_la_core_names_numeric, NLREG);
  hash_reg_names (RCLASS_LAR, riscv_la_core_names_abi, NLREG);
	  ...
	  bfd_reloc_code_real_type *imm_reloc)
{
  ...
  //LACore extension used for parsing Reg/Mem/Scratch locations
  char la_ismem;
  char la_issch;
  ...
  for ( ; insn && insn->name && strcmp (insn->name, str) == 0; insn++) {
    ...
    //printf("DEBUG: insn->name = %s, argsStart = %s\n", insn->name, argsStart);
    ...
    *imm_reloc = BFD_RELOC_UNUSED;
    ...
    for (args = insn->args;; ++args) {
      ...
      s += strspn (s, " \t");
      //printf("DEBUG: args = %s, args[0] = %u\n", args, *args);
      switch (*args) {
        
        case '\0': 	/* End of args.  */
          //printf("DEBUG: end of args!\n");
          if (insn->pinfo != INSN_MACRO) {
            if (!insn->match_func (insn, ip->insn_opcode)){
              //printf("DEBUG: failed match\n");
              break;
            }
            if (riscv_insn_length (insn->match) == 2 && !riscv_opts.rvc) {
              //printf("DEBUG: riscv_insn_length failed\n");
              break;
            }
          }
          if (*s != '\0'){
            //printf("DEBUG: failed zero\n");
            break;
          }
...
rvc_lui:
        ...
        case 'X': //LACore extension
          //printf("DEBUG: X-args = %s\n", args);
          switch (*++args) {
            case 'a':
            case 'b':
            case 'c':
              //printf("DEBUG: X-args = X%s\n", args);
              if (reg_lookup (&s, RCLASS_GPR, &regno)) {
                c = *args;
                if (*s == ' ')
                  ++s;
                switch (c) {
                  case 'a': INSERT_OPERAND (LRXA, *ip, regno); break;
                  case 'b': INSERT_OPERAND (LRXB, *ip, regno); break;
                  case 'c': INSERT_OPERAND (LRXC, *ip, regno); break;
                }
                //printf("DEBUG: X-args found = X%s\n", args);
                continue;
              }
              //printf("DEBUG: X-args not found = X%s\n", args);
              break;
            ...
            case 'B':
            case 'C':
              //printf("DEBUG: ABC-args = X%s, s=%s\n", args, s);
              if (reg_lookup (&s, RCLASS_LAR, &regno)) {
                c = *args;
                if (*s == ' ')
                  ++s;
                switch (c) {
                  case 'A': INSERT_OPERAND (LRA, *ip, regno); break;
                  case 'B': INSERT_OPERAND (LRB, *ip, regno); break;
                  case 'C': INSERT_OPERAND (LRC, *ip, regno); break;
                }
                //printf("DEBUG: ABC-arg found = X%s\n", args);
                continue;
              }
              //printf("DEBUG: ABC-arg not found = X%s, s=%s\n", args, s);
              break;
            ...
            case 'D':
              switch (*++args) {
                case ',':
                  --args;
                  //printf("DEBUG: D-arg (no mem ref) = X%s\n", args);
                  if (reg_lookup (&s, RCLASS_LAR, &regno)) {
                    c = *args;
                    if (*s == ' ')
                      ++s;
                    INSERT_OPERAND (LRD, *ip, regno);
                    //printf("DEBUG: D-arg (no mem ref) found = X%s\n", args);
                    continue;
                  }
                  //printf("DEBUG: D-arg (no mem ref) not found = X%s\n", args);
                  break;

                case 'm':
                  //printf("DEBUG: D-arg (mem ref) = X%s\n", args);
                  la_ismem = 0;
                  la_issch = 0;
                  if(*s == '(') {
                    la_ismem = 1;
                  ...
                  if (reg_lookup (&s, RCLASS_LAR, &regno)) {
                    ...
                    INSERT_OPERAND (LRD, *ip, regno);
                    if(la_ismem){
                      INSERT_OPERAND (LRDLOC, *ip, LA_LOC_MEM);
                    } else if(la_issch){
                      INSERT_OPERAND (LRDLOC, *ip, LA_LOC_SCH);
                    } else {
                      INSERT_OPERAND (LRDLOC, *ip, LA_LOC_REG);
                    }
                  }
                  if((la_ismem && *s != ')') || (la_issch && *s != '}')){
                    as_bad (_("Missing closing } or )\n"));
                  }
                  if(la_ismem || la_issch)
                    ++s;
                  continue;
                defaut:
                    as_bad (_("Invalid character after XD = %s\n"), args);

              }
              break;
              
            default:
              as_bad (_("bad LACore field specifier 'X%s'\n"), args);
          }
          break;
...
jump:
      ...
      s = argsStart;
      //printf("DEBUG: failure, argsStart = %s\n", argsStart);
