...
workload = (dir_path + "/dtrsm_la_core_sweep")
...
def exec_sweep(args, params):
    for simd_width in [4,8]:
        for vec_nodes in [8,16]:
...
def dcache_sweep(args, params):
    for cache_line in [128]:
        for l1d_size_log in [16]:
            for l2_size_log in [18]:
    ...
    for cache_line in [128]:
        for l2_size_log in [18]:
            for banks in [1,2,4,8,16]:
                ...
                for la_size_log in []: #BAD AT DGEMM, IGNORE OPTION
                    params["cache_line"] = str(cache_line)
